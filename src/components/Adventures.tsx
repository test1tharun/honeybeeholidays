...
const Adventures = () => {
  ...
  return (
    <section id="adventures" className="py-8 md:py-20 bg-muted/30">
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6 mb-6 md:mb-12">
          {adventures.map((adventure, index) => (
            <div
              key={adventure.title}
              className="bg-card p-3 md:p-8 rounded-lg shadow-md text-center border border-border hover:shadow-xl hover:border-primary/50 transition-all duration-300 animate-fade-in"
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              <adventure.icon className="h-8 w-8 md:h-16 md:w-16 text-primary mx-auto mb-2 md:mb-4" />
              <h3 className="text-sm md:text-xl font-bold mb-1 md:mb-3 text-foreground">{adventure.title}</h3>
              <p className="text-xs md:text-base text-muted-foreground line-clamp-3 md:line-clamp-none">{adventure.description}</p>
            </div>
          ))}
        </div>
        ...
        <div className="text-center animate-scale-in" style={{ animationDelay: "0.4s" }}>
          <Button
            ...
            size="lg"
            className="bg-primary hover:bg-primary/90 text-secondary font-semibold px-4 md:px-8 py-3 md:py-6 text-sm md:text-lg"
          >
